...
char *receive_response(int sockfd) 
{
    ...
    if (bytes_received <= 0) { free(response); return NULL; }
...
void filter_and_write(char *data_start, ssize_t data_len, FILE *file, int *total_received) {
    ...
    while (remaining_len > 0) {
        ...
        if (found_h && (!found_j || found_h < found_j)) found = found_h;
        else if (found_j) found = found_j;
...
int main() 
{
    ...
    // 1. Set a 20-second receive timeout
    ...
    while (1) 
    {
        ...
        switch (choice) 
        {
            ...
            case 2:
                ...
                if (send_command(sockfd, command) < 0) break;
                ...
                if (strncmp(response, "+OK", 3) == 0)
                {
                    ...
                    while (total_received < file_size)
                    {
                        ...
                        // --- Error / Hang Check Block (bytes_received <= 0) ---
                        if(bytes_received <= 0) 
                        {
                            ...
                            // ⭐️ ULTIMATE COMPLETION CHECK: Trust the closure if we received >99%
                            if (bytes_received == 0 || (bytes_received < 0 && errno != EINTR)) { 
                                // Check if we received at least 99% of the file
                                if (total_received >= file_size * 0.99) { 
                                    fprintf(stderr, "\nWarning: Server closed connection at 99%%. Forcing transfer completion.\n");
                                    total_received = file_size; // Force loop exit
                                    break; 
                                }
                            }
